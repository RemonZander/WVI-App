...
import { AttributeIds, BrowseResult, ClientSession, DataType, DataValue, OPCUAClient, ReferenceDescription, StatusCodes, TimestampsToReturn, UserTokenType, DataTypeIds } from "node-opcua-client";
...
export default class OPCUAclient {
    ...
    private async RecursiveBrowse(browseResult: BrowseResult, session: ClientSession) {
        ...
        for (var a = 0; a < browseResult.references.length; a++) {
            result.push({ browseName: browseResult.references[a].browseName.toString(), NodeId: browseResult.references[a].nodeId.toString(), dataType: "" });
        ...
        try {
            ...
            await this.ReadData(session, nodes).then((data) => {
                for (var a = 0; a < data.length; a++) {
                    ...
                    nodes[a].dataType = data[a].dataType;
                }
            });
    ...
    private async ReadData(session: ClientSession, nodes) {
        ...
        for (var a = 0; a < nodes.length; a++) {
            ...
            await session.read({ nodeId, attributeId: AttributeIds.Value }).then((data) => {
                dataValues.push({ data: data.value.value, dataType: DataTypeIds[data.value.dataType] });
            });
        }
